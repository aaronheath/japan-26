...
it('returns the correct prompt slug', function () {
    $generator = Wrestling::make();

    $reflection = new ReflectionMethod($generator, 'promptSlug');

    expect($reflection->invoke($generator))->toBe('wrestling');
});
...
it('returns correct prompt args with activity', function () {
    ...
    $activity = DayActivity::factory()->for($day)->for($city)->wrestling()->create();

    $generator = Wrestling::make()->activity($activity);
